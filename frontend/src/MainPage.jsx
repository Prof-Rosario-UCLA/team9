    ...
    <div className="h-screen w-screen overflow-hidden relative">
        ...
        <div className="flex-none">
          <div className="dropdown dropdown-end">
            <div tabIndex={0} role="button" className="btn btn-square btn-ghost text-base-content">
              {/* ↑ CHANGED from text-white to text-base-content to adapt to theme */}
      ...
      <div className="flex flex-col md:flex-row h-[calc(100%-4rem)] bg-cover bg-center bg-no-repeat" style={{ backgroundImage: `url(${backgroundImg})` }}>
        ...
        <div className="bg-base-100 w-full md:w-56 flex-shrink-0 flex flex-col justify-between">
          <ul className="menu p-2 rounded-box text-base-content dark:text-info">
            {/* ↑ Sidebar text is base-content in light mode, info blue in dark mode */}
            <li><button className="hover:text-primary" onClick={() => setActivePanel('Profile')}>Profile</button></li>
            <li><button className="hover:text-primary" onClick={() => setActivePanel('Group')}>Group</button></li>
            <li><button className="hover:text-primary" onClick={() => setActivePanel('Statistics')}>Statistics</button></li>
            <li><button className="hover:text-primary" onClick={() => setActivePanel('Calendar2')}>Calendar</button></li>
          </ul>
          ...
          {showTutorial && tutorialStep === 1 && (
            <div className="absolute inset-0 flex items-center justify-center">
              <button className="btn btn-info text-base-100 text-lg px-6 animate-pulse" onClick={() => navigate('/setup')}>
                {/* ↑ CHANGED from text-white to text-base-100 for better contrast in both modes */}
                Get Started
              </button>
            </div>
          )}
