...
export default function CalendarPanel2() {
  ...
  return (
    <main className="w-full h-full flex flex-col p-2 overflow-hidden">
      ...
      <section className="max-w-5xl w-full mx-auto scale-[0.90] sm:scale-[0.95] md:scale-100 md:-translate-y-2 grid grid-cols-7 auto-rows-fr gap-1 sm:gap-2 md:gap-3">
        {[...Array(daysInMonth)].map((_, i) => {
          ...
          if (allCompleted) borderColor = 'border-green-500';
          else if (isPast && hasIncomplete) borderColor = 'border-red-500';
          else if (hasChores) borderColor = 'border-purple-500';
          ...
          return (
            <article
              ...
              style={{ width: '90%', aspectRatio: '1 / 1', margin: 'auto' }}
            >
              <strong>{day}</strong>
              <div className="w-full min-w-0 flex-grow overflow-y-auto" style={{ fontSize: '0.85rem', lineHeight: '1.2' }}>
                {chores.slice(0, 2).map((chore, j) => (
                  <p key={j} className={`truncate ${chore.completed ? 'line-through text-base-content/50' : ''}`}>
                    • {chore.description}
                  </p>
                ))}
                {chores.length > 2 && (
                  <p className="italic text-base-content/70 text-[9px]">+{chores.length - 2} more</p>
                )}
              </div>
            </article>
          );
      ...
      {selectedDatePopup && (
        <aside className="fixed inset-0 bg-black/50 z-50 flex items-center justify-center p-4">
          <section className="bg-base-100 rounded-box p-4 w-full max-w-md text-sm text-base-content border border-info shadow-lg max-h-[90vh] flex flex-col overflow-hidden">
            <header className="text-xl text-info font-bold mb-4 text-center">
            ...
            {selectedDatePopup.chores.length > ITEMS_PER_PAGE && (
              <footer className="join mt-2 self-center">
                <button className="join-item btn btn-sm" disabled={popupPage === 0} onClick={() => setPopupPage(p => Math.max(0, p - 1))}>«</button>
                <button className="join-item btn btn-sm">Page {popupPage + 1} of {Math.ceil(selectedDatePopup.chores.length / ITEMS_PER_PAGE)}</button>
                <button className="join-item btn btn-sm" disabled={popupPage >= Math.ceil(selectedDatePopup.chores.length / ITEMS_PER_PAGE) - 1} onClick={() => setPopupPage(p => p + 1)}>»</button>
              </footer>
            )}

            <button className="btn btn-sm w-full mt-2" onClick={() => setSelectedDatePopup(null)}>Close</button>
          </section>
        </aside>
      )}
      ...
      {selectedChore && (
        <aside className="fixed inset-0 bg-black/50 z-50 flex items-center justify-center p-4">
          <section className="bg-base-100 rounded-box p-4 w-full max-w-md text-sm text-base-content border border-info shadow-lg max-h-[90vh] overflow-auto">
            <header className="text-xl text-info font-bold mb-4 text-center">
            ...
            {!selectedChore.assigned && !selectedChore.completed && (
              <button className="btn btn-info btn-sm text-white w-full mt-4" onClick={handleClaimChore}>Claim this chore</button>
            )}
            {selectedChore.assigned && !selectedChore.completed && (
              <button className="btn btn-success btn-sm text-white w-full mt-2" onClick={handleCompleteChore}>Mark as Completed</button>
            )}
